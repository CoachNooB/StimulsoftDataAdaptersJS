...
exports.process = function (command, onResult) {
  ...
  try {
      ...
      for (var record of result.rows) {
        ...
        for (var columnIndex in record) {
          ...
          if (value instanceof Uint8Array)
            value = Buffer.from(result.rows[recordIndex][columnIndex]).toString(
              "base64"
            );
          ...
          if (parameter) {
            if (parameter.index == null) {
              ...
              if (parameter.typeGroup == "number")
                ...
                parameterValue = new Date(parameter.value);
              else if (parameter.typeGroup == "date")
                parameterValue = new Date(parameter.value);
              else if (parameter.typeGroup == "time")
                parameterValue = new Date(parameter.value);
    ...
    try {
      ...
      utils.CLIENT_INFO.program = utils.CLIENT_INFO.program
        .split("(")
        .join("_")
        .split(")")
        .join("_");
