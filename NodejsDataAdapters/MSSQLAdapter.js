...
exports.process = function (command, onResult) {
  ...
  try {
    ...
    var query = function (queryString, parameters, maxDataRows) {
      ...
      if (command.command == "Execute") request.execute(queryString, onExecute);
      else request.query(queryString, onExecute);
    };
    ...
    var onQuery = function (recordset, maxDataRows) {
      ...
      for (var recordIndex in recordset) {
        ...
        for (var columnName in recordset[recordIndex]) {
          ...
          if (
            recordset[recordIndex][columnName] instanceof Uint8Array ||
          ...
          if (
            ...
            if (types[columnIndex] == "time") {
              recordset[recordIndex][columnName] = dateTime
                .substr(dateTime.indexOf("T") + 1)
                .replace("Z", "");
            } else if (types[columnIndex] == "datetimeZ") {
              var offset = "+00:00";
              recordset[recordIndex][columnName] =
                dateTime.replace("Z", "") + offset;
            } else {
          ...
          if (
            columnName == "" &&
            Array.isArray(recordset[recordIndex][columnName])
          ) {
            for (
              var i = 0;
              i < recordset[recordIndex][columnName].length;
              i++
            ) {
              if (
                columns.length <= columnIndex + i &&
          ...
          } else row[columnIndex] = recordset[recordIndex][columnName];
        }
      ...
      for (var typeIndex in types) {
        if (types[typeIndex] == "datetimeZ")
          types[typeIndex] = "datetimeoffset";
      }
          ...
          if (match && match.length >= 2) {
            ...
            switch (match[0]) {
              ...
              case "server":
                ...
                if ("instanceName" in hostInfo)
                  config.options["instanceName"] = hostInfo.instanceName;
                break;
    ...
    if (!("connectionTimeout" in config) && "timeout" in command)
      config.connectionTimeout = command.timeout;
    if (!("requestTimeout" in config) && "timeout" in command)
      config.requestTimeout = command.timeout;
